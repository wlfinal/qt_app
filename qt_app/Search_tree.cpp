...
void Cboard::AIgo(int &x, int &y) {
	...
	if (step == 0) {
		...
		//己方VCT
		tend_VCT = tend_VCTA;
		for (int depth = 6; depth <= VCT_depth1; depth+=2) {
		...
		//博弈树搜索
		Search_tree(tmp, base_depth2); x = tmp.x; y = tmp.y;
		for (int depth = base_depth2+2; depth <= search_depth1; depth++) {
		...
		//判对方VCT
		tend_VCT = tend_VCTD;
		for (int depth = 4; depth <= VCT_depth2; depth += 2) {
			//cout << depth << endl;
