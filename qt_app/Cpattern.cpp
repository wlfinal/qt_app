...
void Cpattern::Get_bptn() {
	...
	//Q4
	v = T1::Q4;
	for (int i = 0; i < (1 << 7); i++) if (bptn[i] == T1::Q5){
		for (int j = 0; j < 5; j++) {
			hash = i - (1 << j);
			if (bptn[hash]==T1::Q4F) continue;
			bptn[hash] = v;
		}
	}
	...
	//Q3F
	v = T1::Q3F;
	for (int i = 0; i < (1 << 7); i++) if (bptn[i] == T1::Q4F) {
...
//
	//Q3
	v = T1::Q3;
	for (int i = 0; i < (1 << 7); i++) if (bptn[i] == T1::Q4) {
		for (int j = 0; j < 5; j++) if (i&(1 << j)) {
			hash = i - (1 << j);
			if (bptn[hash]) continue;
			bptn[hash] = v;
	...
	//Q2F
	v = T1::Q2F;
	for (int i = 0; i < (1 << 7); i++) if (bptn[i] == T1::Q3F) {
		for (int j = 0; j < 5; j++) if (i&(1 << j)) {
			hash = i - (1 << j);
			if (bptn[hash]) continue;
			bptn[hash] = v;
	...
	//Q2
	v = T1::Q2;
	for (int i = 0; i < (1 << 7); i++) if (bptn[i] == T1::Q3) {
		for (int j = 0; j < 5; j++) if (i&(1 << j)) {
			hash = i - (1 << j);
			if (bptn[hash]) continue;
			bptn[hash] = v;
